...
interface TrackerEvent {
  event: string;
  trackedEntity: string; // ← missing
  enrollment: string; // ← missing
  programStage: string;
  status: string;
  occurredAt?: string;
 ...
 */
  ...
  // ── BACKWARD SYNC: Delta pull ─────────────────────────────
  private readonly storageService = inject(LocalStorageService);

  fetchDeltaEvents(updatedAfter: string, pageSize = 100): Observable<TrackerEvent[]> {
    let params = new HttpParams()
      .set('program', environment.dhis2.program)
      .set('updatedAfter', updatedAfter)
      .set('pageSize', String(pageSize))
      .set('ouMode', 'ACCESSIBLE')
      .set('fields', 'event,trackedEntity,enrollment,updatedAt,programStage,occurredAt,dataValues[dataElement,value]')
      .set('order', 'updatedAt:asc');

    return this.http.get<any>(`${this.base}/tracker/events`, { params }).pipe(
      map(res => (res.instances || res.events || []) as TrackerEvent[])
    );
  }

  async syncAllAccessiblePatients(lastSyncIsoDate: string, orgUnitId: string) {
  // Step 1: Only Ratnapura living - your requirement
  const eventParams = new HttpParams()
    .set('program', environment.dhis2.program)
    .set('programStage', 'x0vRwubw5S7') // MUST add - 1st visit has district
    .set('ouMode', 'ACCESSIBLE')
    .set('ou', orgUnitId) // fix 409
    .set('updatedAfter', lastSyncIsoDate)
    .set('filter', 'iB1RHZOqhhb:EQ:Ratnapura')
    .set('fields', 'trackedEntity,event,updatedAt') // add event

  const eventRes: any = await firstValueFrom(
    this.http.get(`${this.base}/tracker/events`, { params: eventParams })
  )

  const ids = [...new Set(eventRes.instances.map((e:any) => e.trackedEntity))];
  if (!ids.length) return [];

  // Step 2: Fetch TEIs
  const entityParams = new HttpParams()
    .set('program', environment.dhis2.program)
    .set('ou', orgUnitId) // keeps 409 fix
    .set('ouMode', 'ACCESSIBLE')
    .set('trackedEntity', ids.join(';')) // use ; not , for older API, or trackedEntities with ,
    .set('fields', 'trackedEntity,attributes,enrollments[events[dataValues]]')

  const patientRes: any = await firstValueFrom(
    this.http.get(`${this.base}/tracker/trackedEntities`, { params: entityParams })
  )
  return patientRes.instances;
}
  ...
  private getVisitNumberFromStage(stageId: string): number | null {
    const map: Record<string, number> = {
      [environment.PROGRAM_STAGES.STAGE_1]: 1,
      [environment.PROGRAM_STAGES.STAGE_2]: 2,
      [environment.PROGRAM_STAGES.STAGE_3]: 3,
      [environment.PROGRAM_STAGES.STAGE_4]: 4,
      [environment.PROGRAM_STAGES.STAGE_5]: 5,
      [environment.PROGRAM_STAGES.STAGE_6]: 6,
      [environment.PROGRAM_STAGES.STAGE_7]: 7,
      [environment.PROGRAM_STAGES.STAGE_8]: 8,
      [environment.PROGRAM_STAGES.STAGE_9]: 9,
      [environment.PROGRAM_STAGES.STAGE_10]: 10,
      [environment.PROGRAM_STAGES.STAGE_11]: 11,
      [environment.PROGRAM_STAGES.STAGE_12]: 12,
    };
